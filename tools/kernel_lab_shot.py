#!/usr/bin/env python3
"""Kernel Lab start-menu / window screenshots under QEMU.

    python3 tools/kernel_lab_shot.py
"""
...
SHOTS = os.path.join(ROOT, "build", "shots-lab")
...
def main():
    ...
    shot(qemu, "02-start", 0.4)

    # Kernel Lab is start-menu index 4.
    click(qemu, 80, menu_y + 8 + 4 * rh + rh // 2, 1.0)
    shot(qemu, "03-kernel-lab", 0.6)

    mon(qemu, "quit")
    qemu.wait(timeout=10)
    print("lab shots in " + SHOTS)
